...
# - '모든 결정자' 가 '후보키'가 되어야 한다는 조건을 만족하는 정규화
...
/*
    ...
    EX) 학생 테이블 '학번', '이름', '주민번호', '나이', '성별'
		> '학번'(후보키), '나이','성별'을 제거해도 학생 구별이 가능
        > 학번, 주민번호 , 이메일 등
    2) 기본키(Primary Key)
    : 후보키 중 대표로 선택된 키
    - 반드시 NULL이 아니고, 중복되지 않아야 함
    - 하나의 테이블에 반드시 한 개만 존재한다.
    > '학번'
    3) 대체키(Alternate Key)
    ...
    4) 결정자( Determinant)
    : 어떤 속성 집합이 다른 속성을 '결정(함수종속)' 할 수 있다면
		, 해당 속성 집합은 '결정자'
    > '학번' , '주민번호' 등 대부분의 후보키가 결정자가 될 수 있음
*/

# 제3정규화는 비주요 속성이 후보키가 아니어도 허용
# : BCNF는 모든 결정자가 반드시 후보키(유일성, 최소성)여야함

# - 유일성 : 중복 안됨, 최소성 : 최소한의 속성들로만 키를 구성
...
# 과목ID (결정자)
# >> course_name 결정 (course_id는 결정자이긴 하지만 후보키 아님)
# >> BCNF 위반
### 해결방법 (BCNF 예시)
create table 과목(
