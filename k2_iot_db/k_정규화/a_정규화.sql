/*
    ...
    >> 정규화를 하지 않으면, 데이터 중복과이상현상(삽입 / 수정/ 삭제 이상) 발생 가능
    ...
    4. 정규화 필요성
        ...
        >> 위 테이블의 문제점
            ...
            2) 이상 현상 발생
				- 삽입이상: 수강생 없이 강의만 등록 불가
				- 수정 이상: 같은 강사명이나 학생명을 여러 행에 걸쳐 모두 수정
                - 삭제 이상: 학생 삭제 시, 강의 정보 까지 삭제될 위험
	...
	5. 정규화 단계
		1) 제 1 정규형
			: 모든 컬럼이 원자값을 가져야 함
            - 반복되는 그룹을 제거 (EX: 하나의 셀에 여러 값 저장 금지)
		...
		4) BCNF 정규형
            ...
            - 3NF 보다 조금더 엄격한 조건( 모든 결정자가 후보키여야함)
*/
...
CREATE TABLE IF NOT EXISTS `student_course`(
    ...
    primary key (student_id, course_id) # student_id + course_id는 중복 허용 안됨 / 비워질 수 없음
);
...
#이상현상 예시
# 1) 삽입 이상
-- insert into `student_course`(course_id, course_name, couse_instructor)
-- values
-- 	(104,'코리아IT','조승범');

# 2) 수정 이상
# 오류는 없음, 각행에서 조건을 찾아 일일이 변경해야함 >> 메모리 부담 존재
...
show index from `student_course_connects`;
...
##정규화 사용 모범 사례
# 정규화가 무조건적인 정답은 아니다
# : 과동한 정규화는 JOIN이 많아져 성능 저하의 우려가 발생
