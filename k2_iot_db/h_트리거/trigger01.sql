...
-- 데이터 무결성 : 수동으로 처리할 작업을 자동으로 처리해서 데이터 오류 방지 ( 주문 후 결제 테이블 자동 삽입)
...
# 3. 트리거 동작 방식
# 이벤트가 발생할 때만 자동 실행 (call 문으로 직접 실행 불가)
# cf) DML 이벤트 감지(insert, update, delete) - select x 

# 4. 트리거 종류
# 1) BEFORE 트리거 : 해당 작업이 수행되기 전에 실행
# 2) AFTER 트리거 : 해당 작업시 수행된 후에 실행
...
# cf) 주의점
# : 남용 시 디버깅 어렵고, 성능 저하 가능성 존재 >> 필요한 곳에서만 사용 

# 6. 트리거 기본 문법
# : 스토어드 프로시저 유사(call 문 사용 금지)
...
# 트리거(trigger) VS 트랜젝션(transaction)
# 1) 트리거
# : 이벤트 발생 시 '자동 처리'
# - insert update delete 이벤트 발생 시에만 
# - 자동 실행, 개발자가 직접 제어 불가
# > 자동화 된 응답 처리, 감시
# EX) 게시글 수정 시로그 자동 기록
...
# - 개발자가 직접 명시적으로 시작
# - COMMIT(적용시점), ROLLBACK(취소)로 직접 제어
# > 데이터의 일관성 보장, 오류 복구
# EX) 주문 처리 중 오류 시 전체 작업 롤백

# cf) 원자성(Atomicity): 모두성공 또는 모두 실패(하나라도 실패시 전체 실패)
