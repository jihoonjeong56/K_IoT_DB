/*
	===JOIN (결합하다)
    : 두 개 이상의 테이블을 연결하여 하나의 결과 집합을 만들어 내는 SQL 기능
    ...
    - 정확하게 일치하는 데이터만 보고 싶을 때
    2) 외부조인(OUTER JOIN)
    : 조건이 불일치해도 포함(합집합 NULL 포함)
    - 빠진 데이터를 확인하거나 전체 조회시 
    3) 상호 조인(CROSS JOIN)
    : 모든 조합반환( 곱집합) - A테이블 * B테이블
    - 두 테이블의 모든 집합
    4) 자체 조인(SELF JOIN)
    : 자기 자신과의 조인(자신과 비교)
    - 상위/하위 관계 , 이전/다음 비교
...
/*
    ...
    FROM 기준 테이블(첫 번째 테이블) AS 별칭
		INNER JOIN 조인테이블(두번째 테이블) AS 별칭
        ON 조인될 조건( 결합 조건)
    [WHERE 조건식 ....];
...
WHERE P.member_id = 1;
# 조인 시 두 개 이상의 테이블에서 동일한 열 이름이 존재하는 경우
#		>> 테이블명(별칭).열 이름 형식으로 표기
...
# : 구매 이력이 있는 회원만 조회
SELECT
	DISTINCT M.name, M.contact # 조인한 테이블의 데이터가 반드시 조회될 필요는 없다.
FROM 
	`members` M
    JOIN `purchases` P
    ON M.member_id = P.member_id;
# cf) 서브쿼리(구매이력이 있는 회원 조회)
...
# 2) 기준 테이블 선탱 : 주로 1(일) 테이블을 기준으로 조인
# 3) 조인 순서 FROM A JOIN B != FROM B JOIN A (결과는 같이만 기준이 다름)
