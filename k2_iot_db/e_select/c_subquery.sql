/*
	===서브쿼리(subQuery)===
    : 메인 쿼리 내부에서 실행되는 하위 쿼리(중첩쿼리)
    - () 괄호 안에 작성하여 필요한 값을 먼저 추출한 뒤 메인 쿼리에 필요한 데이터를 동적으로 제공해주는 역할
    
    == 특징 ==
    1) select, from, where 등 다양한 곳에서 사용 가능
    2) 하나의 값(단일 행) 또는 여러값(다중행)을 반환 가능
    
    == 필요성 == 
    ? 어떤 사람의 포인트가 가장 많을지
    ? 평균보다 높은 포인트를 가진 사람은 누구인지
...
# - 서브쿼리를 통해 제외할 대상을 돟적으로 구해오는 방식

# 차이점
# > 조건이 단수하고 고정 : 빠르고 간단 (서브쿼리 없는것)
# > 조건이 복잡하거나 동적으로 바뀔 수있음 : 더 유연하고 안전 - NOT IN (서브쿼리) 
