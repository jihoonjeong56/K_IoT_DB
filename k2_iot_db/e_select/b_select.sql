...
/*
	4. group by
    : 그룹으로 묶어주는 역할
    - 여러 행을 그룹화 하여 집계함수를 적용해 데이터 단일화에 주로 사용
    
    cf) 집계 함수 : 그룹화된 데이터에 대한 계산
    - max(), min(), avg(), sum()
    - count() : 행의 수를 반환
    - count(distinct): 행의 수를 반화( + 중복은 1개만 인정)
*/
#select * from `members`
#group by grade;
...
# 2) 지역펼 평균 포인트 계산
select area_code, avg(points) -- 평균은 실수 반환
from `members`
group by
 area_code;
...
/*
	5. having
    : group by 함께 사용, 그룹화 된 결가에 대한 조건 지정
    - where 조건과 식은 유사 , 하지만 그룹화 된 데이터에 대한 조건 지정
*/
# 총인원이 2명 이상인 등급 조회
select  grade, count(*) from `members`
group by
...
group by
 ...
 /*
    ...
    : 데팅터 정렬
     -결과의 값이나 개수에 영향 없음
     - 기본값 asc(ascending, 오름차룬) , desc(descending, 내림차순)
 */
...
#cf)정렬된 데이터를 기반으로 추가정렬(콤마로 정렬 상태 나열)
select * from `members`
order by
...
limit 5;
select * from `members`
limit 4 offset 2; -- offset 2 : 세번째 행부터 출력
