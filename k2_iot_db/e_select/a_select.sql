/*
    ...
    > WHERE(필터링) > group by (그룹화) > having(조건) > select(컬럼추출)
...
# cf) 두 개 이상의 컬럼 조회 시 , 로 구분하여 나열
select
	`member_id`, `name`, `contact`
from
	`members`;

# cf) alias 별칭 부여 조쇠(as 키워드)
# : 별칭 부여하지 않을 경우 테이블 생성 시 지정한 컬럼명으로 조회한다
# - 컬럼 명이 변경 x 조회 당시에 사용
# - 공백 사용 시 따옴표로 반드시 지정
select
	`member_id` as 고유번호, `name`as '화원이름', `contact` as '회원 연락처'
from
	`members`;
...
-- where
-- 	points = null;
# cf) null값은 값이 없음을 나타냄 : 어떤 값과도 비교하거나 연산이 불가
# 	>> is null, is not null 연산자 사용 여부 확인 가능(true/false)

# A is null : null인 경우 true, 아닌 경우 false
# A is not null : null인경우 false, 아닌 경우  ture;
select * from `members`
where 
	points is null;
...
select * from `members`
where area_code in('Seoul', 'Busan');
# > 문자열 데이터에 대한 or식 간소화
...
#%(퍼센트) : 무엇이든 허용(0개 이상의 임의의 무자 공간을 나타냄)
select * from `members`;

# 시작이 J 이고, 뒤는 0개 이상의 문자를 허용 검색 : J%
select * from `members`
where
	name like 'J%';
...
# 어떤 문자열 내에서든 'un'이 포함만 되면 허용하는 검색 %un%
select * from `members`
where
name like '%un%';

# 1개의 임의의 문자 + u + 0개이상의 임의의 문자 허용 검색 : 이름의 두번째 글자가 u인 모든 회원 조회
select * from `members`
where
	name like '_u%';
#이름이 4글자인 모든 회원;
