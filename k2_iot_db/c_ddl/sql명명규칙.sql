# MySQL 명명규칙
# : DB, 테이블, 컬럼 명명 규칙alter

/*
	1. 대소문자 구분 없음
    : 일관성 있는 작성 권장
    - 소문자로 작성 후 ctrl shift u를 통해 대문자변환
    
    2. 길이 제한
    : 최대 64자 까지 사용가능
    
    3. 문자 제한
    : 문자, 숫자, _만 사용가능(공백, 기타 특수 기호 사용 안됨)
    
    4. 숫자로 시작할 수 없다
    5. MySQL 예약어는 사용안함(권장) 
    : 부득이하게 사용하는 경우``(백틱)으로 묶어서 사용가능
*/
#------------------------------#
# cf) SQL 내 명명 시 '반드시' 의미있는 이름 사용 권장
...
SELECT `student_name` FROM `busan high school`;

#	>> 혹시모를 예약어와의 충돌이나 외부 시스템 연동 시 문제 방지를 위해 백틱을 사용하는 것이 안정적

# 2) 따옴표
#	: 문자열 데이터를 저장하는데 사용(문자열 리터럴 표현)
#	- DB, 테이블, 컬럼명으로는 사용하지 않는 것을 권장
#	- 실제 데이터임을 명시
SELECT 'price'FROM `products`; -- 가격 컬럼이 아닌 실제 price 문자열(글자)로 인식